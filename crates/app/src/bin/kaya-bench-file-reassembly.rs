use kaya_files::{chunk_bytes, reassemble_chunks, FileMetadata, FileTransferConfig};
...
fn main() {
    ...
    let reassembled = reassemble_chunks(&metadata, &chunks).expect("reassembled");
    let elapsed = started.elapsed();
    let mb = reassembled.len() as f64 / (1024.0 * 1024.0);
    let seconds = elapsed.as_secs_f64().max(0.001);

    println!(
        "file_reassembly: size_mb={} chunks={} elapsed_ms={} mb_per_sec={:.2}",
        size_mb,
        chunks.len(),
        elapsed.as_millis(),
        mb / seconds
